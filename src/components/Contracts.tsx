...
export default function Contracts({
  ...
  return (
    <div className="fade-up">
        ...
        {contracts.map((c, i) => {
          ...
          return (
            <div
              ...
              className="fade-up group flex cursor-pointer flex-col border border-line bg-surface p-5 text-left transition-all duration-300 hover:-translate-y-0.5 hover:border-brand/50 hover:shadow-[0_14px_34px_-16px_rgba(14,36,60,0.3)]"
              style={{ animationDelay: `${i * 60}ms` }}
            >
              <div className="flex items-start justify-between">
                <div className="grid h-10 w-10 place-items-center border border-line bg-soft text-brand transition-colors group-hover:bg-brand group-hover:text-white">
              ...
              <p className="mt-4 font-display text-[15px] font-bold text-ink group-hover:text-brand">№ {c.number}</p>
              ...
              <div className="mt-1 flex items-center justify-between">
                ...
                <span className="flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                  <button onClick={(e) => { e.stopPropagation(); setEditing(c); }} className="cursor-pointer border border-line p-1.5 text-mut hover:border-navy hover:text-navy" title="Изменить">
                    <IconPencil size={13} />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); onDelete(c.id); }} className="cursor-pointer border border-line p-1.5 text-mut hover:border-danger hover:text-danger" title="Удалить">
                    <IconTrash size={13} />
                  </button>
                </span>
              </div>
      ...
      {editing !== null && (
        <ContractForm initial={editing === "new" ? null : editing} parties={parties} onSave={(c) => { onUpsert(c); setEditing(null); }} onClose={() => setEditing(null)} />
      )}
    </div>
  );
}
