...
export function rowToParty(r: DbParty): Party {
  return {
    ...
    person: r.person || undefined, representativePosition: r.representative_position || undefined, representativeName: r.representative_name || undefined, representativeFull: r.representative_full || undefined,
...
export function partyToRow(p: Party, orgId: string): Partial<DbParty> {
  return {
    ...
    person: p.person ?? "", representative_position: p.representativePosition ?? "", representative_name: p.representativeName ?? "", representative_full: p.representativeFull ?? "",
...
export function rowToPayment(r: DbPayment): Payment {
  return { id: r.id, docId: r.doc_id ?? "", contractId: r.contract_id || undefined, counterpartyId: r.counterparty_id || undefined, date: r.date, amount: r.amount, method: r.method, name: r.name, direction: r.direction ?? "income" };
}

export function paymentToRow(p: Payment, orgId: string): Partial<DbPayment> {
  return { org_id: orgId, doc_id: p.docId || null, contract_id: p.contractId ?? null, counterparty_id: p.counterpartyId ?? null, date: p.date, amount: p.amount, method: p.method, name: p.name, direction: p.direction ?? "income" };
}
